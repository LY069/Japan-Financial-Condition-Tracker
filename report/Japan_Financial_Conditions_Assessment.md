...
> **⚠️ Figures update note (read first).** This narrative was drafted on the initial snapshot. Two methodology upgrades and a data load have since changed several headline numbers — the live dashboard and `web/data.json` are authoritative. Current values: **composite FCI ≈ +0.80**; **headline real-rate axis +0.19 "mildly accommodative"** (now scored over the *longest available* ex-post history, 1974/86+, with 2005+ ex-ante kept as a reference); **natural-rate band loaded from the real BoJ six-model series (1992Q1–2025Q3)** — latest **low −0.94% / mid −0.36% / high +0.53%**, so the real policy rate (−0.98%) sits **just below the band** (gap to midpoint ≈ **−0.62pp**). Numbers in the body below that predate these changes (e.g. FCI +0.69, real-rate −0.04, midpoint −0.15%) are superseded by these; a full prose refresh is pending.
...
> **What "restrictive" is measured against here.** The funding-costs axis (lending rate, CP issuance rate, AA corporate-bond spread) is a z-score versus **2005-onward history** — the *same* baseline caveat that applies to the 2005+ real-rate reference. Because 2005–2024 is dominated by zero/negative-rate policy, average lending and CP rates over that window are very low, so today's normalizing rates score *above* that depressed mean and read "restrictive" even though, in level terms, ~1.1–1.2% lending rates are historically low. Unlike the real-rate axis, these BoJ series are not readily available far enough back to re-baseline on a multi-decade window, so the axis stays on 2005+; interpret it as "tighter than the deflation-era norm," not "tight in absolute terms." The benign corporate-bond spread confirms there is no credit-risk tightening.
